...
class OrderLineItem(models.Model):
    ...
    edible_product = models.ForeignKey(
        EdibleProduct, null=True, blank=True, on_delete=models.CASCADE
    )
    merch_product = models.ForeignKey(
        MerchProduct, null=True, blank=True, on_delete=models.CASCADE
    )
    subscription_product = models.ForeignKey(
        SubscriptionProduct, null=True, blank=True, on_delete=models.CASCADE
    )
    product_type = models.CharField(max_length=20, null=True, blank=True)

    weight = models.IntegerField(null=True, blank=True)  # Stores the weight
    quantity = models.IntegerField(null=False, blank=False, default=0)
    selected_text = models.ForeignKey(
        TextOption, null=True, blank=True, on_delete=models.SET_NULL
    )
    ...
    def save(self, *args, **kwargs):
        if not self.id:
            if self.product_type == 'edible' and self.weight:
                ...
                self.lineitem_total = self.merch_product.price * Decimal(
                    self.quantity
                )
            elif self.product_type == 'subscription':
                self.lineitem_total = (
                    self.subscription_product.price * Decimal(
                        self.quantity
                    )
                )
    ...
    def __str__(self):
        if self.product_type == 'edible':
            return (
                f'{self.edible_product.flavour} ({self.weight}g) on order '
                f'{self.order.order_number}'
            )
        elif self.product_type == 'merch':
            text = f" -{self.selected_text.text}" if self.selected_text else ""
            return (
                f'{self.merch_product.name}{text} on order '
                f'{self.order.order_number}'
            )
        else:
